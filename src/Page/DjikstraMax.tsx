...
// Dijkstra Max avec étapes pour la démonstration
function dijkstraMaxWithSteps(nodes: string[], edges: Edge[], start: string) {
  const dist: Record<string, number> = {};
  const prev: Record<string, string | null> = {};
  const visited = new Set<string>();
  const steps: DijkstraMaxStep[] = [];
...
function DemoTableMax({ steps, nodeList, endNode }: { steps: DijkstraMaxStep[]; nodeList: string[]; endNode: string }) {
  ...
  return (
    <div className="mt-4 overflow-x-auto">
      <h3 className="text-sm font-semibold text-amber-400 mb-3 flex items-center gap-2">
        ...
        Tableau de démonstration — Dijkstra Max
      </h3>
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-slate-700/60">
            ...
            {sortedNodes.map((n) => (
              <th key={n} className={`border border-slate-600 px-3 py-2 text-center font-medium ${n === endNode ? "text-amber-400" : "text-slate-300"}`}>
                d({n})
              </th>
            ))}
          </tr>
              ...
              <tr key={idx} className={`${idx % 2 === 0 ? "bg-slate-800/40" : "bg-slate-800/70"} hover:bg-slate-700/50 transition-colors`}>
                ...
                <td className="border border-slate-600 px-3 py-2">
                  <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${
                    isInit ? "bg-amber-900/50 text-amber-300 border border-amber-500/30"
...
export default function DijkstraMax() {
  ...
  const [steps, setSteps]       = useState<DijkstraMaxStep[]>([]);
  ...
  function runLongestPath() {
    ...
    // Generate demo steps
    const { steps: dSteps } = dijkstraMaxWithSteps(nodeList, edges, startNode);
    setSteps(dSteps);
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-6 max-w-2xl mt-[60px] mx-auto">
      ...
      {result && mode === "demo" && steps.length > 0 && (
        <div className="bg-slate-800 rounded-xl border border-amber-700 p-4 mt-4">
          <DemoTableMax steps={steps} nodeList={nodeList} endNode={endNode} />
        </div>
      )}
    </div>
  );
